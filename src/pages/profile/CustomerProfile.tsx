...
import type { Address, CustomerProfile as Profile } from "../../types/profile";
...
export default function CustomerProfile() {
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      setMe(draft);
      setSnack({ open:true, msg:"Profile saved", sev:"success" });
    } catch (e:any) {
  ...
  const onPickAvatar = async (file: File) => {
    ...
    try {
      ...
      setSnack({ open:true, msg:e.message || "Avatar upload failed", sev:"error" });
    }
  };
        ...
        <Grid container spacing={2}>
          ...
          <Grid size={12}>
            <Box sx={{ display:"flex", gap: 2, justifyContent:"flex-end" }}>
              <Button onClick={()=>setDraft(me!)} disabled={saving}>Reset</Button>
              <Button variant="contained" onClick={saveAll} disabled={saving}>Save changes</Button>
            </Box>
          </Grid>
        </Grid>
      </Container>
